...
// Use the useQuery hook with conditional fetching based on route.params.id
...
watch(blogData, (newValue) => {
  ...
  breadcrumbs.value.push({
    label: blogDataRef?.value?.attributes?.title,
    link: ROUTE_CONSTANTS.BLOG + "/" + route.params?.id,
  });
});
</script>
...
<template>
  ...
  <template v-else>
    ...
    >
      ...
      <div class="text-[#0a102d] text-left text-2xl relative self-stretch font-normal">{{ blogDataRef?.attributes?.title }}</div>
      <div
        class="text-[#6e6e6e] text-left text-base leading-[30px] font-normal relative self-stretch"
      >
        {{ blogDataRef?.attributes?.description }}
      </div>
    </div>
  </template>
</template>
