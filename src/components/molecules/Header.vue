  ...
  <div class="bg-primary-color  common-padding">
    ...
    >
      ...
      >
        ...
        >
          <RouterLink class="cursor-pointer hover:text-tahiti font-bold" :to="link.path">
            {{ link.label }}
          </RouterLink>
        </div>
      </div>
    ...
    >
      <div
        v-for="(link, index) in NAV_LINK"
        :key="index"
        class="text-left font-['DmSans-Regular',_sans-serif] text-base leading-5 font-bold relative"
      >
        <RouterLink class="cursor-pointer" :to="link.path" @click="toggleMobileView">
          {{ link.label }}
        </RouterLink>
      </div>
    </div>
